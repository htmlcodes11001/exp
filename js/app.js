...
function renderPanel(ficha) {
  ...
  panel.innerHTML = `
    <div class="hoja" id="hoja-imprimible">
      ...
      <div class="header">
        <div class="header-title">📁 Expediente de personaje</div>
...
async function confirmarEliminar(ficha) {
  const ok = confirm(`¿Eliminar la ficha de "${ficha.nombre || 'este personaje'}"? Esta acción no se puede deshacer.`);
  if (!ok) return;
  await eliminarFicha(ficha.id);
  fichas = fichas.filter((f) => f.id !== ficha.id);
  fichaActivaId = null;
  renderLista();
  document.getElementById('panel').innerHTML = '<div id="ficha-vacia">Selecciona un personaje o crea uno nuevo para empezar.</div>';
}
...
async function exportarPDF(ficha) {
  const blob = generarPDFDeFicha({
    titulo: ficha.nombre ? `Expediente: ${ficha.nombre}` : 'Expediente de personaje',
